...
def add_softmax_cross_entropy_loss_for_each_scale(scales_to_logits,
  ...
  for scale, logits in six.iteritems(scales_to_logits):
    ...
    # Note(leiquan): Add weight
    scaled_labels = tf.reshape(scaled_labels, shape=[-1])
    irgore_weight = 0
    label0_weight = 97  #background
    label1_weight = 3   #object1
    #label2_weight = 6   #object1
    #label3_weight = 1   #object1
    #label4_weight = 4   #object1
    #label5_weight = 1   #object1
    #label6_weight = 3   #object1
    #label7_weight = 2   #object1
    #label8_weight = 1   #object1

    #not_ignore_mask = tf.to_float(tf.equal(scaled_labels, 0)) * label0_weight + tf.to_float(tf.equal(scaled_labels, 1)) * label1_weight + tf.to_float(tf.equal(scaled_labels, 2)) * label2_weight + tf.to_float(tf.equal(scaled_labels, 3)) * label3_weight + tf.to_float(tf.equal(scaled_labels, 4)) * label4_weight + tf.to_float(tf.equal(scaled_labels, 5)) * label5_weight + tf.to_float(tf.equal(scaled_labels, 6)) * label6_weight + tf.to_float(tf.equal(scaled_labels, 7)) * label7_weight + tf.to_float(tf.equal(scaled_labels, 8)) * label8_weight + tf.to_float(tf.equal(scaled_labels, ignore_label)) * irgore_weight
